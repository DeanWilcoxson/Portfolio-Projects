...
let behemoth = new Demon(
    ...
    250,
    250,
    "Rampage: Tramples over everything in its path! (500)", [fira, blizzara, thundara],
    true
);
let serpentKing = new Demon(
    ...
    200,
    200,
    "Constrictor: Inflicts poison to enemies! (50*10)", [fira, blizzara, thundara],
    true
);
            ...
            for (let item = 0; item < player.inventory.items.length; item++) {
                if (player.inventory.items[item] === potion || items[item] === bomb) {
                    if (useItem == 1) {
                        ...
                        if (itemUse == 1) {} else if (itemUse == 2) {} else if (itemUse == 3) {
                            goBerserk();
                        }
                    } else if (useItem == 2) {
                        goBerserk();
...
function wander() {
    console.log(
        "You Run, until the screaming fades. You stumble into a farmers barn, you fall onto a pile of hay 5"
    );
    readline.keyIn("");
}
